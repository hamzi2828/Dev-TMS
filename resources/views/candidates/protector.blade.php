<x-dashboard :title="$title">
    ...
    <div class="container-p-x flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header border-bottom pt-3 pb-3 d-flex justify-content-between align-items-center">
                ...
                <div>
                    ...
                    <a href="javascript:void(0)" onclick="downloadExcel('Protector Candidates List')"
                    ...
                    @if(count ($candidates) > 0)
                        @foreach($candidates as $candidate)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ $candidate -> protector ? route ('candidates.protectors.edit', ['candidate' => $candidate -> id, 'protector' => $candidate -> protector -> id]) : route ('candidates.protectors.create', ['candidate' => $candidate -> id]) }}">
                                        {{ $candidate -> SerialNo() }}
                                    </a>
                                </td>
                                ...
                                <td>
                                    ...
                                    <span class="badge bg-{{ $bg }}">
                                        {{ !empty(trim ($candidate -> protector ?-> status)) ? str () -> upper ($candidate -> protector ?-> status) : 'In Process' }}
                                    </span>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        @if($candidate -> protector)
                                            ...
                                            @endcan
                                        @else
                                            @can('create', \App\Models\CandidateProtector::class)
                                                <a href="{{ route ('candidates.protectors.create', ['candidate' => $candidate -> id]) }}"
                                                   class="text-body" data-bs-toggle="tooltip"
                                                   data-bs-placement="top"
                                                   data-bs-custom-class="tooltip-primary"
                                                   title="Edit">
        ...
        @include('candidates.popups.protector')
    </div>
    <!-- / Content -->
    @push('scripts')
