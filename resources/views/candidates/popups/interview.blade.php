<div class="modal fade" id="candidateStatusModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered1 modal-simple">
        <div class="modal-content p-0">
            ...
            <form method="post" action="{{ route ('interviews.bulk-status-update') }}">
                @csrf
                <div class="modal-body pt-3">
                    ...
                    <div class="col-12">
                        ...
                        <select name="status" class="form-control select2" data-placeholder="Select"
                                ...
                                id="status">
                            <option></option>
                            <option value="selected" @selected(old ('status') == 'selected')>Selected</option>
                            <option value="rejected" @selected(old ('status') == 'rejected')>Rejected</option>
                            <option value="standby" @selected(old ('status') == 'standBy')>StandBy</option>
                        </select>
                    </div>
                </div>
