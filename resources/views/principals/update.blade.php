            ...
            <div class="col-md-12">
                <div class="card mb-4">
                    ...
                    <form class="pt-0" method="post"
                          action="{{ route ('principals.update', ['principal' => $principal -> id]) }}"
                            ...
                            <div class="row">
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="text" required="required" autofocus="autofocus" class="form-control"
                                           value="{{ old ('title', $principal -> name) }}"
                                           id="title" name="title" />
                                </div>
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="text" class="form-control"
                                           value="{{ old ('contact', $principal -> contact) }}"
                                ...
                                <div class="col-md-3 mb-3">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        @if(!empty(trim ($principal -> file)))
                                            <div>
                                                <a href="{{ $principal -> file }}"
                                                   download="{{ $principal -> file }}"
                                                   target="_blank">
                                                    <i class="tf-icons ti ti-download"></i>
                                                </a>
                                                <a href="{{ $principal -> file }}" target="_blank">
                                ...
                                <div class="col-md-3 mb-3">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        @if(!empty(trim ($principal -> footer)))
                                            <div>
                                                <a href="{{ $principal -> footer }}"
                                                   download="{{ $principal -> footer }}"
                                                   target="_blank">
                                                    <i class="tf-icons ti ti-download"></i>
                                                </a>
                                                <a href="{{ $principal -> footer }}" target="_blank">
                                                    <i class="tf-icons ti ti-photo"></i>
                                                </a>
                                            </div>
                                        @endif
                                                    ...
                                                    @if(count ($country -> cities) > 0)
                                                        @foreach($country -> cities as $city)
                                                            <option value="{{ $city -> id }}"
                                                                    @selected(old ('city-id', $principal -> city_id) == $city -> id)>
                                                                {{ $city -> title }}
                                                            </option>
                                                        @endforeach
                                                    @endif
                                ...
                                <div class="col-md-9 mb-3">
                                    ...
                                    <textarea rows="3" class="form-control"
                                              id="address"
                                              name="address">{{ old ('address', $principal -> address) }}</textarea>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <table class="table table-hover table-sm table-bordered" id="excel-table">
                                            ...
                                            @if(count ($jobs) > 0)
                                                @foreach($jobs as $key => $job)
                                                    @php
                                                        $principalFee = \App\Models\PrincipalJob::where(['principal_id' => $principal -> id, 'job_id' => $job -> id]) -> first();
                                                    @endphp
                                                    <tr>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <label class="w-100">
                                                                <input type="number" class="form-control" name="fee[]"
                                                                       value="{{ old ('fee.'.$key, $principalFee ?-> fee) }}">
                                                            </label>
                                                        </td>
                                                    </tr>
                                                @endforeach
