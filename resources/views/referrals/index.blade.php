<x-dashboard :title="$title">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        @can('edit', $referral)
                                            <a href="{{ route ('referrals.edit', ['referral' => $referral -> id]) }}"
                                        ...
                                        @can('edit', $referral)
                                            <form method="post" id="delete-record-form-{{ $referral -> id }}"
                                                  action="{{ route ('referrals.destroy', ['referral' => $referral -> id]) }}">
                                                @method('DELETE')
                                                @csrf
                                                <button type="button" data-bs-toggle="tooltip"
                                                        ...
                                                        onclick="delete_confirmation({{ $referral -> id }})">
    ...
    @push('scripts')
        <script type="text/javascript">
            init_datatable ( '{{ route ('referrals.create') }}' )
        </script>
    @endpush
</x-dashboard>