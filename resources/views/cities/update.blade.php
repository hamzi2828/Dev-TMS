        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    ...
                    <form class="pt-0" method="post" action="{{ route ('cities.update', ['city' => $city -> id]) }}">
                        @csrf
                        @method('PUT')
                        <div class="card-body pt-1 pb-1">
                            <div class="row">
                                <div class="col-4 mb-3">
                                    ...
                                    <select name="country-id" id="country" class="form-control select2"
                                            data-placeholder="Select" required="required"
                                            data-allow-clear="true">
                                        <option></option>
                                        @if(count ($countries) > 0)
                                            @foreach($countries as $country)
                                                <option value="{{ $country -> id }}" @selected(old ('country-id', $city -> country_id) == $country -> id)>
                                                    {{ $country -> title }}
                                                </option>
                                            @endforeach
                                        @endif
