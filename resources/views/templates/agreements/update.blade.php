            ...
            <div class="col-md-12">
                <div class="card mb-4">
                    ...
                    <form class="pt-0" method="post"
                          action="{{ route ('agreements.update', ['agreement' => $agreement -> id]) }}">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="job-id" class="form-control select2" data-placeholder="Select"
                                        ...
                                        @if(count ($jobs) > 0)
                                            @foreach($jobs as $job)
                                                <option value="{{ $job -> id }}"
                                                        @selected(old ('job-id', $agreement -> job_id) == $job -> id)>
                                                    {{ $job -> title }}
                                                </option>
                                            @endforeach
                                        @endif
                                        ...
                                        @if(count ($principals) > 0)
                                            @foreach($principals as $principal)
                                                <option value="{{ $principal -> id }}"
                                                        @selected(old ('principal-id', $agreement -> principal_id) == $principal -> id)>
                                                    {{ $principal -> name }}
                                                </option>
                                            @endforeach
                                        @endif
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea name="template" id="editor" cols="30"
                                              rows="10">{{ old ('template', $agreement -> template) }}</textarea>
                                    </label>
                                </div>
                            </div>
                        </div>
        ...
        <script type="text/javascript">
            ClassicEditor
                .create ( document.querySelector ( '#editor' ), {
                    toolbar: [ 'heading', '|', 'bold', 'italic', 'link', 'insertTable', 'blockQuote', 'bulletedList', 'numberedList', 'sourceEditing' ],
                } )
                .then ( editor => {
